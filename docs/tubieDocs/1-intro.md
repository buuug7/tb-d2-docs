...
5. 如果想运行普通模式, 运行客户端中 `4_普通模式启动.bat` 启动游戏
...
如果运行非高清版本, 请手动创建 D2Loader.exe 的快捷方式, 在目标中添加 `-w -direct -plugin d2helper.dll -multiopen -depfix -plugin d2hd.dll -plugin map/d2hackmap.dll`, 兼容性中选择以 window XP(sp3) 模式运行, 勾选以管理员身份运行此程序.
