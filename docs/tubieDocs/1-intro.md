...
4. 如果想运行高清模式, 运行客户端中 `1_高清模式启动.bat` 或者 `2_高清模式启动.bat`, 那个效果好用那个
5. 如果想运行普通模式, 运行客户端中 `3_普通模式启动.bat` 启动游戏
...
如果运行非高清版本, 请手动创建 `D2Loader-high.exe` 的快捷方式, 在目标中添加 `-w -nohide -pdir MAP -direct`, 兼容性中选择以 window XP(sp3) 模式运行, 勾选以管理员身份运行此程序.
